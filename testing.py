...
#herustica=matrizHeuristicaLocal(distancesFromCoords())

#feromona=np.array(explo_matrizFeromonaInicial(distancesFromCoords(),1000))
#print(np.sum(feromona==0)) Funciona la feromona los unicos 100 valores en 100 son la diagonal


#myArray=np.array([[1,2,3,4],[4,5,6,7],[7,8,9,7],[1,3,5,7]])
#myArray2=np.full(np.shape(myArray),2)


#p_actual=generatePath(herustica,feromona,1,1)
#print(p_actual)
#PROBANDO LA SUMA DE UNA COLUMNA CON NUMPY
#prueba=myArray[:,0]
#print(prueba)
#print(np.sum(prueba))

#Agregar fila
#myArray=np.append(myArray,[[7,8,9]],axis=0)
#myArray=np.vstack((myArray,[7,8,9]))